...
w_vec_0 = [ 0 0 1;                                           % rotational axis matrix
    1 0 0;
    1 0 0;
    0 1 0;
    1 0 0;
...
while j<1000
    for i=1:num_of_pts                                                      % repeat num_of_pts times
        ...
        [T_a,~,~] = FK_new(twist_matrix,theta_random_vec(i,:));             % Ta calculation, to simulate the measured data
    ...
    if norm(dp) < 10e-6                                                     % check if it has convergenced
        converge_flag = 1;
        break;
    ...
    draw_manipulator(twist_matrix_0,T_matrix(twist_matrix_0(:,end)),'b',1); % draw robot frame with norminal twist
    drawnow;
end
%% plot again
...
for i = 1:number_of_validation_samples
    ...
    T_n = T_n*T_matrix(twist_matrix_0(:,end));
    T_a = T_a*gst_a;
    
    deviation = log_my(T_a/T_n);                                        % calculate deviation between nominal and actual posture
